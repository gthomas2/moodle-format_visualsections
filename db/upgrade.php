...
/**
 * Upgrade script for format_visualsections
 *
 * @param int $oldversion the version we are upgrading from
 * @return bool result
 */
function xmldb_format_visualsections_upgrade($oldversion) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2017020200) {
        ...
        upgrade_plugin_savepoint(true, 2017020200, 'format', 'visualsections');
    }
    ...
    if ($oldversion < 2018030900) {
        ...
        (SELECT id FROM {course} WHERE format=?)", ['visualsections']);

        upgrade_plugin_savepoint(true, 2018030900, 'format', 'visualsections');
    }

    if ($oldversion < 2020022812) {
